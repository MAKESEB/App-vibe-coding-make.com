...
function expect(op) {
  const out = params(op.url).map(field);
...
json(path.join(appDir, "modules/watch-events/metadata.json"), { name: "watchEvents", label: "Watch Events", description: "Triggers when selected Actaport webhook events are delivered.", connection: conn, type: "instant_trigger", typeId: 10, webhook: "actaport-event-webhook" });
json(path.join(appDir, "modules/watch-events/api.imljson"), { response: { output: "{{body}}" } });
